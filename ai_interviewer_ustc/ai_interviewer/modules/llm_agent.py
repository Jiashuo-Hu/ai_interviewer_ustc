...
def llm_stream_chat(history, user_input, domain="cs", interviewer_style=DEFAULT_STYLE):
    """流式对话：先检索上下文，再按风格生成提问。"""

    style_key = interviewer_style
    style_cfg = INTERVIEWER_STYLES.get(style_key, INTERVIEWER_STYLES[DEFAULT_STYLE])

    # RAG 检索上下文
    context = get_retrieved_context(user_input, domain=domain)
